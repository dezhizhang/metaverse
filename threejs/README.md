...
混合部署方案
1. 解决多个企业有不同主题logo等需求
2. 解决多个企业有不同定制化需求
3. 解决用户私有化部署问题

如何解决
1. 前端单独起一个混合云仓库把混合云的项目都放入到该仓库下（单一仓库单一分支只是通过运营后台来控制模块的显隐）
2. 主题问题通过管理后台主题配置功能进行主题的配置，然后在前端通用组件库里通过接口获取到主题配置文件，通过context上下文传递到不同的系统进行主题更改。
3. 多个企业有不同定制化需求如何搞定，通过运营管理后台增加模块和功能开关，不同的企业需要不同的功能模块在运营管理后台进行对应的开启或关闭，然后前端在通用组件库里获取到对应的开关通过context传递到不同系统控制。
4. 用户私有化部署如何搞定，在前端通用组件库调用后端接口进行免登录把cooke注入进去，解析一个微应用域名在运营后台配置然后通过context进行传递下去替换。
