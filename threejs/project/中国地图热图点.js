...
loader.load('/微博签到数据.json',(data) => {
  const coordinates = data.coordinates;
  const vertices = [];
  for(let i=0;i < coordinates.length;i+=2) {
    vertices.push(coordinates[i],coordinates[i + 1],0);
  }

  const geometry = new THREE.BufferGeometry();
  const attribue = new THREE.BufferAttribute(new Float32Array(vertices),3);
