...
function line(pointArr) {
  const newPointArr = [];
  newPointArr.push(pointArr[0],pointArr[1],pointArr[2]);
  for(let i=3;i < pointArr.length;i+=3) {
    newPointArr.push(newPointArr[i],newPointArr[i + 1],newPointArr[i + 2],pointArr[i],pointArr[i+1],pointArr[i + 2]);
  }
  newPointArr.push(pointArr[0],pointArr[1],pointArr[2]);

  const geometry = new THREE.BufferGeometry();
  const vertices = new Float32Array(newPointArr);
